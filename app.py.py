                ...
                if 'issue date' in cell_value and bond_data['issue_date'] is None:
                    try:
                        ...
                        if isinstance(issue_date_val, datetime):
                            bond_data['issue_date'] = issue_date_val.date()
                        elif isinstance(issue_date_val, str):
                            bond_data['issue_date'] = datetime.strptime(issue_date_val.split()[0], '%Y-%m-%d').date()
                        else:
                            bond_data['issue_date'] = issue_date_val
                    except:
                        pass
...
def extract_cashflow_data(df, bond_data):
    ...
    # Look for cash flow headers
    cashflow_dates = []
    cashflow_amounts = []
    
    # Search for cash flow dates column (typically column G or H)
    for col in range(df.shape[1]):
        for row in range(min(20, df.shape[0])):
            cell_value = str(df.iat[row, col]).strip().lower()
            if 'cash flow dates' in cell_value:
                # Found cash flow dates column, extract dates from below rows
                date_col = col
                amount_col = col + 2  # Cash flows are typically 2 columns to the right
                
                # Extract dates and amounts
                for r in range(row + 1, min(row + 50, df.shape[0])):
                    try:
                        date_val = df.iat[r, date_col]
                        amount_val = df.iat[r, amount_col]
                        ...
                        try:
                            ...
                            if amount_val != 0:
                                cashflow_dates.append(date_val)
                                cashflow_amounts.append(amount_val)
                        except:
                            pass
...
def calculate_bond_cashflows(bond_data, face_value, trade_date=None):
    ...
    for i, (date, amount) in enumerate(zip(cashflow_dates, cashflow_amounts)):
        ...
        if isinstance(date, date):
            ...
            # Handle string dates
            try:
                date_obj = datetime.strptime(str(date).split()[0], '%Y-%m-%d').date()
                time_to_payment = (date_obj - trade_date).days / 365.25
            except:
                continue
        ...
        if time_to_payment > 0:
            ...
            cashflows.append({
                'Period': i + 1,
                'Date': date.strftime('%Y-%m-%d') if isinstance(date, date) else str(date),
                'Cash Flow': amount,
...
def generate_cashflows_from_params(bond_data, face_value, trade_date, ytm):
    ...
    # If issue_date is a string, convert to date
    if isinstance(issue_date, str):
        try:
            issue_date = datetime.strptime(issue_date.split()[0], '%Y-%m-%d').date()
        except:
                ...
                if calculate_btn:
                    with st.spinner("Calculating bond valuation..."):
                        try:
                            ...
                            st.error("Please check if the Excel file has proper cash flow data in the expected format.")
